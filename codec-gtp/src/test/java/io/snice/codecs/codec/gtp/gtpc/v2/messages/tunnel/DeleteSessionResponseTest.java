...
public class DeleteSessionResponseTest extends GtpTestBase {

    @Test
    public void testDeleteSessionRequest() {
        final var request = DeleteSessionRequest.create().build();
        final var response = request.createResponse().build();
        assertThat(response.isDeleteSessionResponse(), is(true));
    }
}
