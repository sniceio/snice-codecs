...
public class ImmutableDiameterHeader implements DiameterHeader {
    ...
    public static DiameterHeader frame(final ReadableBuffer buffer) throws DiameterParseException {
    ...
    public DiameterHeader.Builder copy() {
        ...
        return new Builder(copy, true, true);
    }
    ...
    private static class Builder implements DiameterHeader.Builder {
        ...
        // TODO: should we make it optional to use a secure random?
        private static final Random random = new Random(System.currentTimeMillis());
        private final WritableBuffer buffer;
        private boolean hopToHopIdSetByUser = false;
        private boolean endToEndIdSetByUser = false;

        private Builder(final WritableBuffer buffer, boolean hopToHopIdSetByUser, boolean endToEndIdSetByUser) {
            this.buffer = buffer;
            this.hopToHopIdSetByUser = hopToHopIdSetByUser;
            this.endToEndIdSetByUser = endToEndIdSetByUser;
            buffer.fastForwardWriterIndex();
        }
        ...
        public DiameterHeader build() {
            ...
            // TODO: need to keep track of if the EndToEnd has been called or not
            if (!hopToHopIdSetByUser) {
                withHopToHopId(random.nextLong());
            }

            if (!endToEndIdSetByUser) {
        ...
        @Override
        public Builder copy() {
            final var clone = (WritableBuffer)buffer.clone();
