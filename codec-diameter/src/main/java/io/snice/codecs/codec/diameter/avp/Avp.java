...
public interface Avp<T extends DiameterType> extends FramedAvp {
    ...
    static <T extends DiameterType> ValueStep<T> ofType(final Class<T> type) {
        assertNotNull(type);
        return new DefaultValueStepBuilder<>(type);
        /*
        return value -> {
            assertNotNull(value);
    ...
    interface ValueStep<T extends DiameterType> {
        ...
        AvpCodeStep<T> withValue(String value);
        AvpCodeStep<T> withValue(Buffer value);
        AvpCodeStep<T> withValue(int value);
        AvpCodeStep<T> withValue(long value);
    }
    ...
    class DefaultValueStepBuilder<T extends DiameterType> implements ValueStep<T> {

        private final Class<T> type;

        private DefaultValueStepBuilder(final Class<T> type) {
            this.type = type;
        }

        @Override
        public AvpCodeStep<T> withValue(T value) {
            return code -> new DefaultBuilder(value, code);
        }

        @Override
        public AvpCodeStep<T> withValue(String value) {
            return code -> new DefaultBuilder(create(type, value), code);
        }

        @Override
        public AvpCodeStep<T> withValue(Buffer value) {
            return code -> new DefaultBuilder(create(type, value), code);
        }

        @Override
        public AvpCodeStep<T> withValue(int value) {
            return code -> new DefaultBuilder(create(type, value), code);
        }

        @Override
        public AvpCodeStep<T> withValue(long value) {
            return code -> new DefaultBuilder(create(type, value), code);
        }
    }
