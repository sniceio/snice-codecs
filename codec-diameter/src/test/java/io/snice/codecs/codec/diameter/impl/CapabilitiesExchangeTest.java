...
import static io.snice.codecs.codec.diameter.avp.api.ResultCode.DiameterErrorEapCodeUnknown5048;
import static io.snice.codecs.codec.diameter.avp.api.ResultCode.DiameterSuccess2001;
import static java.util.Optional.empty;
import static org.hamcrest.CoreMatchers.is;
import static org.hamcrest.CoreMatchers.not;
import static org.hamcrest.MatcherAssert.assertThat;

public class CapabilitiesExchangeTest extends io.snice.codecs.codec.diameter.DiameterTestBase {
    ...
    public void testCreateCER() {
        ...
        assertThat(apps.getFramedAvp(AuthApplicationId.CODE).get().ensure(), is(authId));
        assertThat(apps.getFramedAvp(AcctApplicationId.CODE).get().ensure(), is(acctId));
        assertThat(apps.getFramedAvp(VendorId.CODE).get().ensure(), is(vendorId));
    }

    @Test
    public void testCreateCEA() {
        final var host = "seagull.node.epc.mnc001.mcc001.3gppnetwork.org";
        final var realm = "epc.mnc001.mcc001.3gppnetwork.org";
        final var cea = cer.createAnswer(DiameterSuccess2001)
                .withOriginHost(host)
                .withOriginRealm(realm)
                .build();
        assertThat(cea.isCEA(), is(true));
        assertThat(cea.getResultCode().get(), is(DiameterSuccess2001));
        assertThat(cea.getOriginHost().getValue().asString(), is(host));
        assertThat(cea.getOriginRealm().getValue().asString(), is(realm));
        assertThat(cea.getDestinationHost(), is(Optional.empty()));
        assertThat(cea.getDestinationRealm(), is(Optional.empty()));
